...
const jobRoles = [
	...
	},
	{
		...
		description: 'Design intuitive and accessible user experiences for our digital products. Create wireframes, prototypes, and user flows.',
		responsibilities:
...
app.get('/job-roles', (req, res) => {
	try {
		...
		if (statusName && typeof statusName === 'string') {
			const filtered = jobRoles.filter(role => role.status.toLowerCase() === statusName.toLowerCase());
