...
test.describe('Add a new job role', () => {
  ...
  test('displays the add job role form', async ({ page }) => {
    await expect(page.getByRole('textbox', { name: 'Job role name' })).toBeVisible();
    await expect(page.locator('#description')).toBeVisible();
    await expect(page.locator('#responsibilities')).toBeVisible();
    await expect(page.getByRole('textbox', { name: 'SharePoint link' })).toBeVisible();
  ...
  test('contains blank form fields', async ({ page }) => {
    ...
    await expect(page.getByRole('textbox', { name: 'SharePoint link' })).toHaveValue('');
  ...
  test('adds new role and navigates to /job-roles on valid data submit', async ({ page }) => {
    ...
    await page.getByRole('textbox', { name: 'SharePoint link' }).fill('https://example.sharepoint.com');
    ...
    await page.locator('#bandId').selectOption('4a535fab-694b-42a4-ac71-c70f1b927785');
